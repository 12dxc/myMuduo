...
{
    setState(kConnected);
    channel_->tie(shared_from_this()); // 留住TcpConnection对象，以防Channel使用回调时未定义
    ...
    if (channel_->isWriting())
    {
        ...
        if (n > 0)
        {
            ...
            if (outputBuffer_.readableBytes() == 0)
            {
                ...
                {
                    shutdownInLoop(); // 在当前loop删除TcpConnection
                }
            }
        }
        else
...
void TcpConnection::handleClose()
{
    ...
    TcpConnectionPtr condPtr(shared_from_this());
    connectionCallback_(condPtr); // 执行连接关闭的回调
    closeCallback_(condPtr);      // 关闭连接的回调  执行的是TcpServer::removeConnection回调方法
}
